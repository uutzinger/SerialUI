...
WORKFLOW_FILE="${WORKFLOW_FILE:-build-linux.yml}"
REF="${REF:-$(git rev-parse --abbrev-ref HEAD)}"
DOWNLOAD_DIR="${DOWNLOAD_DIR:-artifacts/linux-runner}"
COPY_TO_DIST="${COPY_TO_DIST:-1}"

usage() {
  cat <<EOF
Usage:
  scripts/run_github_linux_build.sh [options]

Options:
  --ref <git-ref>            Branch/tag ref to run on (default: current branch)
  --workflow <file-or-name>  Workflow id/name (default: build-linux.yml)
  --download-dir <dir>       Local directory for downloaded artifacts
                             (default: artifacts/linux-runner)
