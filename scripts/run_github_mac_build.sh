...
WORKFLOW_FILE="${WORKFLOW_FILE:-build-macos.yml}"
REF="${REF:-$(git rev-parse --abbrev-ref HEAD)}"
DOWNLOAD_DIR="${DOWNLOAD_DIR:-artifacts/macos-runner}"
COPY_TO_DIST="${COPY_TO_DIST:-1}"

usage() {
  cat <<EOF
Usage:
  scripts/run_github_mac_build.sh [options]

Options:
  --ref <git-ref>            Branch/tag ref to run on (default: current branch)
  --workflow <file-or-name>  Workflow id/name (default: build-macos.yml)
  --download-dir <dir>       Local directory for downloaded artifacts
                             (default: artifacts/macos-runner)
