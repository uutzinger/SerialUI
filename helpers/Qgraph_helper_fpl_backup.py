...
from config import ( MAX_ROWS, MAX_COLS, MAX_ROWS_LINEDATA,
                     ...
                     COLORS, AXIS_FONT_COLOR, AXIS_COLOR, GRID_COLOR, 
                     FRAME_PLANE_COLOR, FRAME_TITLE_COLOR, LEGEND_FONT_COLOR,
                     GRID_MINOR_COLOR, GRID_ALPHA, TICK_COLOR, 
...
class QChart(QObject):
    ...
    Slots (functions available to respond to external signals)
        ...
        on_pg_viewBox_changed(vb, ranges, axes) called when user zooms or pans the plot
    ...
    def __init__(self, parent=None, ui=None):
        ...
        self.plotterPage  = self.ui.findChild(QWidget,        "tab_Plotter") 
        self.monitorPage  = self.ui.findChild(QWidget,        "tab_Monitor") 
    ...
    def fpl_figure_init(self):
        ...
        if not getattr(self, "chartFPLInitialized", False):
            ...
            # Visibility check
            if not (self.chartView.isVisible() 
                    and self.chartView.size().width() > 10 
                    and self.chartView.size().height() > 10):
                QTimer.singleShot(100, self.fpl_figure_init)                   # try again later
                return
            ...
            if self.initChartFPL():
                ...
                QTimer.singleShot(500, self.pg_figure_init)
        else:
            self.logSignal.emit(logging.WARNING, 
    ...
    def initChartFPL(self) -> bool:
        ...
        # forces fastplotlib to use an OpenGL compatible adapter.
        ...
        self.canvasFPL = self.fpl_fig.show(autoscale=True, maintain_aspect=True)
        self.canvasFPL.setContentsMargins(0, 0, 0, 0)
        layoutFPL.addWidget(self.canvasFPL)

        # Create the Fastplotlib subplot
        self.fpl_subplot = self.fpl_fig[0, 0]
        self.fpl_subplot.axes.visible = True
        self.fpl_subplot.background_color = CHART_BACKGROUND_COLOR

        # Camera for the subplot
        self.fpl_camera = self.fpl_subplot.camera

        # Set axis & tick colors
        axes = self.fpl_subplot.axes
        for axis in (axes.x, axes.y):
            line = getattr(axis, "line", None)
            if line and hasattr(line, "material"):
                line.material.color = AXIS_COLOR
            # Pre-color tick primitives if already constructed
            for part_name, clr in (
                ("ticks", TICK_COLOR),
                ("points", TICK_COLOR),
                ("text", AXIS_FONT_COLOR),
            ):
                part = getattr(axis, part_name, None)
                if part and hasattr(part, "material"):
                    try:
                        part.material.color = clr
                    except Exception:
                        pass

        # Set figure background color (affects title bar and figure tools background)
        self.fpl_subplot.frame.plane.material.color = FRAME_PLANE_COLOR
        ...
        self.fpl_subplot.title.face_color = FRAME_TITLE_COLOR
        self.fpl_subplot.title.outline_color = FRAME_TITLE_COLOR
        ...
        self.fpl_subplot.docks["bottom"].add_text(
            "Sample",
            font_size=16,
            face_color=AXIS_FONT_COLOR,
            anchor="middle-center",
            offset=(0, 0, 0),
        )
        self.fpl_subplot.docks["bottom"].background_color = CHART_BACKGROUND_COLOR
        ...
        self.fpl_subplot.docks["left"].background_color = CHART_BACKGROUND_COLOR

        # Axes
        ax = self.fpl_subplot.axes.x
        ax.thickness = AXIS_LINEWIDTH
        if ax.line and hasattr(ax.line, "material"):
            ax.line.material.color = AXIS_COLOR
        if ax.ticks  is not None: ax.ticks.material.color  = TICK_COLOR
        if ax.points is not None: ax.points.material.color = TICK_COLOR
        if ax.text   is not None: ax.text.material.color   = TICK_COLOR

        ax = self.fpl_subplot.axes.y
        ax.thickness = AXIS_LINEWIDTH
        if ax.line and hasattr(ax.line, "material"):
            ax.line.material.color = AXIS_COLOR
        if ax.ticks  is not None: ax.ticks.material.color  = TICK_COLOR
        ...
        if self.fpl_subplot.axes.grids:
            gxy = self.fpl_subplot.axes.grids.xy
            gxy.visible = True
            gxy.color   = GRID_COLOR
            gxy.axis_color  = GRID_COLOR                                       # Axis color
            gxy.major_color = GRID_COLOR                                       # Major grid color
            gxy.minor_color = GRID_MINOR_COLOR                                 # Minor grid color
            gxy.major_thickness = AXIS_LINEWIDTH                               # Major grid thickness
            gxy.minor_thickness = AXIS_LINEWIDTH/2                             # Minor grid thickness
        ...
        self.fpl_subplot.axes.x.start_pos = (x_min, y_min, 0.0)
        self.fpl_subplot.axes.x.end_pos   = (x_max, y_min, 0.0)
        # Location of y-axis in scene
        self.fpl_subplot.axes.y.start_pos = (x_min, y_min, 0.0)
        self.fpl_subplot.axes.y.end_pos   = (x_min, y_max, 0.0)

        # Axis limits
        self.fpl_subplot.axes.x_limits = (x_min, x_max)
        self.fpl_subplot.axes.y_limits = (y_min, y_max)
    ...
    def fpl_createLegend(self):
        ...
        legend_dock.background_color = LEGEND_BACKGROUND_COLOR
        ...
        return Legend(
            ...
            background_color=LEGEND_BACKGROUND_COLOR,                          # optional: the background color of the legend
            label_color = LEGEND_FONT_COLOR,
        )
    ...
    def fpl_updateLegend(self, lines, labels ):
        ...
        for line, label in zip(lines, labels):
            self.legend.add_graphic(line, label, label_color = LEGEND_FONT_COLOR)
    ...
    def fpl_updateAxesTicks(
        ...
        if lo is None or hi is None:
            ...
            #     (_x_lo, _x_hi) = (ax.x.start_pos, ax.x.end_pos)
            #     (_y_lo, _y_hi) = (ax.y.start_pos, ax.y.end_pos)
            #     x_lo = _x_lo[0]
            #     y_lo = _y_lo[1]
            #     x_hi = _x_hi[0]
            #     y_hi = _y_hi[1]
    ...
    def updatePlot(self) -> None:
            ...
            for idx in range(delta_data_traces):
                if not USE_FASTPLOTLIB:
                    ...
                    new_data_trace = self.fpl_subplot.add_line(
                        ...
                        colors=self.pensFPL[(idx+num_traces) % len(self.pensFPL)], # color for the line
        ...
        if not USE_FASTPLOTLIB:
            ...
            for i in range(num_cols):
                ...
                if num_valid_samples > capacity:
                    ...
                    # Since we only obtain maxPoints which is same as capacity, this should never execute
                    x = x[-capacity:]
                    y = y[-capacity:]
                    ...
                    # Since we trim data, there always should be enough room available
        ...
        if not USE_FASTPLOTLIB:
            ...
            if not prev_ranges:
                ...
                # Previous values
                # ----------------------------------------
                x_min_prev, x_max_prev, x_span_prev, x_center_prev, x_limits_lo_prev, x_limits_hi_prev = prev_ranges.get("x", (None, None, None, None, None, None))
                y_min_prev, y_max_prev, y_span_prev, y_center_prev, y_limits_lo_prev, y_limits_hi_prev = prev_ranges.get("y", (None, None, None, None, None, None))
                x_limits_changed = False
                x_span_changed   = False
                ...
                self.fpl_last_ranges = {
                    "x": (x_min_prev, x_max_prev, x_span_prev, x_center_prev, x_limits_lo_prev, x_limits_hi_prev), 
                    "y": (y_min_prev, y_max_prev, y_span_prev, y_center_prev, y_limits_lo_prev, y_limits_hi_prev)}
    ...
    def on_pushButton_ChartStartStop(self) -> None:
        ...
        if self.ui.pushButton_ChartStartStop.text() == "Start":
            ...
            if not USE_FASTPLOTLIB:
                ...
                # set pan/zoom limits once (±50% headroom + zoom bounds like in your PG path)
                x_span = max(1.0, float(self.x_max - self.x_min))
                y_span = max(SMALLEST, float(self.y_max - self.y_min))
                pad_frac = 0.5
                x_pad = x_span * pad_frac
                ...
                # If your axes expose min/max range, set them here to match your 1%/25% policy:
                min_frac = 0.01                                                # don’t allow zooming in beyond 1% of data span
                max_frac = 4.00                                                # don’t allow zooming out so data < 25% of the view
    ...
    def on_pushButton_ChartClear(self) -> None:
        ...
        if not USE_FASTPLOTLIB:
            ...
            for data_trace in self.data_traces:
                # self.canvasFPL.removeItem(data_trace)
                self.fpl_subplot.delete_graphic(data_trace)
            self.data_traces.clear()
            self.fpl_clearLegend()
            self.data_traces_writeidx = []
