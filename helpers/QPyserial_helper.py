...
NUM_CHARS_COLLATE      = 48          # [chars] serial buffer should be read after number of characters
SERIAL_BUFFER_SIZE     = 4096        # [bytes] size of the serial buffer, CAN NOT CHANGE on Linux and Darwin
MAX_RECEIVER_INTERVAL  = 100         # [ms], will check if new data is available at least 10 times per second
MIN_RECEIVER_INTERVAL  = 0           # [ms], with 0 checks serial port each idle cycle
TRIM_INTERVAL          = 30000       # [ms] interval to trim text display window
FLUSH_INTERVAL_MS      = 33          # [ms] 30 Hz update of the text display (received data is buffered)
########################################################################################
...
class QPlainTextEditExtended(QObject):
    ...
    def __init__(self, widget, lines_in_buffer: int = MAX_TEXTBROWSER_LENGTH):
        ...
        self.trimBuffer = deque(maxlen=lines_in_buffer)
        ...
        self.mtoc_appendTextNoNL = 0
        self.mtoc_appendHtmlNoNL = 0
        self.mtoc_clear = 0
        self.mtoc_trim = 0 
    ...
    def handle_mtoc(self):
        ...
        log_message = textwrap.dedent(f"""
            ...
            appendTextNoNL          took {self.mtoc_appendTextNoNL*1000:.2f} ms.
            appendHtmlNoNL          took {self.mtoc_appendHtmlNoNL*1000:.2f} ms.
            clear                   took {self.mtoc_clear*1000:.2f} ms.
            trim                    took {self.mtoc_trim*1000:.2f} ms.
        """)
    ...
    def appendTextNoNL(self, text: str, check_nl: bool = False):
        ...
        if text[-1] != "\n":  # If last character is NOT a newline, it's incomplete
            ...
            self.incomplete_line = ""  
        self.trimBuffer.extend(lines) # store in buffer

        # Performance optimization: (does not make difference)
        self.widget.setUpdatesEnabled(False)
        # self.widget.blockSignals(True)  # Disable updates
        ...
        # self.widget.blockSignals(False)  # Disable updates
        ...
        if PROFILEME: 
            ...
            self.mtoc_appendTextNoNL = max((toc - tic),self.mtoc_appendTextNoNL )  # End performance tracking
    ...
    def appendHtmlNoNL(self, html: str):
        ...
        if PROFILEME: 
            toc = time.perf_counter()  # End performance tracking
            self.mtoc_appendHtmlNoNL = max((toc - tic),self.mtoc_appendHtmlNoNL )  # End performance tracking
    ...
    def clear(self):
        ...
        self.widget.clear()
        if self.usePlain:
            self.trimBuffer.clear()

        if PROFILEME: 
            toc = time.perf_counter()  # End performance tracking
            self.mtoc_clear = max((toc - tic),self.mtoc_clear )  # End performance tracking
    ...
    def trim(self):
        ...
        if PROFILEME: 
            toc = time.perf_counter()  # End performance tracking
            self.mtoc_trim = max((toc - tic),self.mtoc_trim )  # End performance tracking
    ...
    def _trimText(self):
        ...
        if text_display_line_count > self.textBrowserLength:
            ...
            text = "\n".join(self.trimBuffer)  # Efficiently rebuild the display content
...
class USBMonitorWorker(QObject):
    ...
    def handle_mtoc(self):
        ...
        log_message = textwrap.dedent(f"""
            USB Monitor
            =============================================================
            monitor_usb             took {self.monitor_usb*1000:.2f} ms.
        """)
        self.handle_log(logging.INFO, log_message)
...
class QSerialUI(QObject):
    ...
    def __init__(self, parent=None, ui=None, worker=None, logger=None):
        ...
        self.ui.plainTextEdit_Text_Ext = QPlainTextEditExtended(
            widget = self.ui.plainTextEdit_Text, 
            lines_in_buffer = MAX_TEXTBROWSER_LENGTH)
    ...
    def flushByteArrayBuffer(self):
        ...
        if self.byteArrayBuffer:
            ...
            if self.record:
                try:
                    self.recordingFile.write(bytes(self.byteArrayBuffer))
                except Exception as e:
            ...
            # Decode byte array
            text = self._safe_decode(self.byteArrayBuffer, self.encoding)
    ...
    def flushLinesBuffer(self):
        ...
        if self.linesBuffer:
            ...
            if self.record:
                try:
                    self.recordingFile.writelines(self.linesBuffer)
                except Exception as e:
                    self.handle_log(logging.ERROR, f"[{self.thread_id}]: could not write to file {self.recordingFileName}. Error: {e}")
                    self.record = False
                    self.ui.radioButton_SerialRecord.setChecked(self.record)
            ...
            decoded_lines = [self._safe_decode(line, self.encoding) for line in self.linesBuffer]
    ...
    def on_throughputReady(self, numReceived, numSent):
        ...
        rx = numReceived - self.lastNumReceived # delat RX per second
        tx = numSent - self.lastNumSent         # delta TX per second
        if rx >=0: self.rx = rx
        if tx >=0: self.tx = tx
        # # poor man's low pass, not needed
    ...
    def cleanup(self):
        """
        Perform cleanup tasks for QSerialUI, such as 
        stopping timers, 
...
# Q Serial
#
# This is a separate thread handling serial input and output.
...
class QSerial(QObject):
    ...
    def __init__(self, parent=None):
        ...
        self.mtoc_on_setupReceiverRequest = 0.
        self.mtoc_updateReceiver = 0.
    ...
    def handle_mtoc(self):
        ...
        log_message = textwrap.dedent(f"""
            QSerial Profiling
            =============================================================
            on_setupReceiverRequest took {self.mtoc_on_setupReceiverRequest*1000:.2f} ms.
            ...
            updateReceiver          took {self.mtoc_updateReceiver*1000:.2f} ms.
            Serial Read             took {self.PSer.mtoc_read*1000:.2f} ms.
            Serial Write            took {self.PSer.mtoc_write*1000:.2f} ms.
            Serial Readline         took {self.PSer.mtoc_readline*1000:.2f} ms.
            Serial Writeline        took {self.PSer.mtoc_writeline*1000:.2f} ms.
            Serial Readlines        took {self.PSer.mtoc_readlines*1000:.2f} ms.
        """)
        self.handle_log(logging.INFO, log_message)
    ...
    def on_setupReceiverRequest(self):
        ...
        if PROFILEME: 
            toc = time.perf_counter()
            self.mtoc_on_setupReceiverRequest = max((toc - tic), self.mtoc_on_setupReceiverRequest)  # End performance tracking
    ...
    def on_startReceiverRequest(self):
        ...
        serialReadTimeOut, receiverInterval, receiverIntervalStandby = (
            self.compute_timeouts(self.PSer.baud, num_chars = NUM_CHARS_COLLATE)
        ) 
    ...
    def updateReceiver(self):
        ...
        if not data_or_lines:  # nothing received
            ...
            if PROFILEME: 
                toc = time.perf_counter()
                self.mtoc_updateReceiver = max((toc - tic), self.mtoc_updateReceiver)
        ...
        if PROFILEME: 
            toc = time.perf_counter()
            self.mtoc_updateReceiver = max((toc - tic), self.mtoc_updateReceiver)  # End performance tracking
    ...
    def on_changePortRequest(self, port: str, baud: int, esp_reset: bool):
        ...
        if port != "":
            ...
            if self.PSer.open(
                ...
                self.handle_log(
                    logging.INFO,
                    f"[{self.thread_id}]: port {port} opened with baud {baud} eol {repr(self.textLineTerminator)} timeout {self.PSer.timeout}, receiver interval {self.receiverInterval} and standby interval {self.receiverIntervalStandby} ms."
                )
            else:
                self.handle_log(
                    logging.ERROR,
    ...
    def compute_timeouts(self, baud: int, num_chars: int = NUM_CHARS_COLLATE) -> tuple:

        if baud == None or baud <= 0:
            baud = DEFAULT_BAUDRATE

        # Set timeout to wait for text
        serialReadTimeOut = 0  # make it non blocking

        # Set the QTimer interval so that each call we get NUM_CHARS_COLLATE characters
        receiverInterval = floor(num_chars * 10 / baud * 1000)

        # Lets make sure intervale does not result in serial buffer overflow 
        receiverIntervalStandby = floor(SERIAL_BUFFER_SIZE * 10 / baud * 1000)
...
class PSerial():
    ...
    def __init__(self, parent=None):
        ...
        self.mtoc_read = 0.
        self.mtoc_write = 0.
        self.mtoc_readline = 0.
        self.mtoc_writeline = 0.
    ...
    def open(self, port: str, baud: int, eol: bytes, timeout: float, esp_reset: bool) -> bool:
        ...
        self.set_serial_buffer(self.ser, rx_size=SERIAL_BUFFER_SIZE, tx_size=SERIAL_BUFFER_SIZE)
    ...
    def set_serial_buffer(self, ser, rx_size=4096, tx_size=4096):
        """
        ...
        try:
            ...
            #     fcntl.ioctl(ser.fileno(), termios.TIOCOUTQ, struct.pack("I", rx_size))
                ...
                self.handle_log(logging.INFO, f"[SER            ]: Can not change buffer size on {platform.system()}")
                return
    ...
    def read(self) -> bytes:
        ...
        if PROFILEME: 
            toc = time.perf_counter()
            self.mtoc_read = max((toc - tic), self.mtoc_read)  # End performance tracking
    ...
    def readline(self) -> bytes:
        ...
        try:
            ...
            if PROFILEME: 
                toc = time.perf_counter()
                self.mtoc_readline = max((toc - tic), self.mtoc_readline)  # End performance tracking
    ...
    def readlines(self) -> list:
        ...
        if not self._ser_open:
            ...
            if PROFILEME: 
                toc = time.perf_counter()
                self.mtoc_readlines = max((toc - tic), self.mtoc_readlines)  # End performance tracking
            return []
    ...
    def write(self, byte_array: bytes) -> int:
        ...
        try:
            ...
            if PROFILEME: 
                toc = time.perf_counter()
                self.mtoc_write = max((toc - tic), self.mtoc_write)  # End performance tracking
            return 0
